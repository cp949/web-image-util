# Release Checklist

버전 업데이트와 npm 배포 전에 문서와 실제 패키지 상태가 어긋나지 않도록 확인하는 체크리스트다.

## 버전 동기화

패키지 버전의 단일 출처는 `sub/web-image-util/package.json`의 `version`이다.

새 버전으로 올릴 때는 다음을 확인한다.

- `sub/web-image-util/package.json`의 `version`을 새 배포 버전으로 갱신한다.
- 사용자에게 의미 있는 변경이면 `sub/web-image-util/CHANGELOG.md`에 같은 버전 섹션을 추가한다.
- 저장소에서 별도로 버전을 추적하는 내부 가이드 문서가 있다면 같은 버전으로 맞춘다.

루트 `README.md`와 `sub/web-image-util/README.md`에는 정적 버전 문자열을 두지 않는다. npm 페이지가 버전을 자동으로 노출하므로, README에 `현재 배포 준비 버전` 같은 수동 동기화 지점을 만들지 않는다.

버전 문자열을 바꾼 뒤 아래 명령으로 README 계열 문서에 정적 버전이 남아 있지 않은지 확인한다. 루트 README에는 npm version badge도 두지 않는다.

```bash
rg -n "[0-9]+\\.[0-9]+\\.[0-9]+|현재 배포 준비 버전" README.md sub/web-image-util/README.md
rg -n "npm version" README.md
```

## 변경 내역(CHANGELOG)

사용자에게 의미 있는 변경이 있으면 `sub/web-image-util/CHANGELOG.md`에 같은 버전 섹션을 추가한다. Keep a Changelog 형식을 따르며 추가/변경/수정/호환성 깨지는 변경으로 분류한다.

CHANGELOG에는 구현 세부보다 사용자가 알아야 할 API 변경, 공개 export 변화, 검증 범위, 호환성 변경을 우선 적는다.

## 스크립트와 문서 동기화

명령어 문서를 수정하거나 `package.json` scripts를 바꿨다면 실제 스크립트와 문서가 같은지 확인한다.

```bash
node -p "Object.keys(require('./package.json').scripts).sort().join('\n')"
node -p "Object.keys(require('./sub/web-image-util/package.json').scripts).sort().join('\n')"
rg -n "pnpm (build|build:watch|dev|dev:exam|typecheck|lint|lint:fix|format|format:check|verify:ci|verify:release|publish:npm|test|test:scripts|test:coverage|test:browser|test:contract|test:package-subpath)" README.md sub/web-image-util/README.md docs
```

현재 루트 CI 성격의 기본 검증 명령은 `pnpm verify:ci`다. `verify:ci`는 루트 운영 스크립트 테스트인 `pnpm test:scripts`를 먼저 실행한 뒤 Turbo 기반 타입체크, lint, format 검사, Node 테스트, 계약 테스트를 실행한다. 릴리스 전 필수 검증 명령은 `pnpm verify:release`이며, `verify:ci`, browser smoke test, npm pack dry-run을 함께 실행한다. `test:coverage`는 별도 점검 경로이며, `verify:ci`에 포함하기로 결정한 경우에만 문서의 coverage 설명을 바꾼다.

루트 운영 스크립트 테스트는 `tests/unit/scripts/**`에 둔다. `sub/web-image-util/tests/**`는 패키지 라이브러리 테스트를 담당하므로, 루트 `scripts/**`의 테스트를 패키지 내부로 되돌리지 않는다.

## 구조 설명 동기화

아키텍처 문서에서 파일명이나 클래스명을 언급할 때는 실제 파일 존재 여부를 확인한다.

```bash
rg -n "pipeline\\.ts|RenderPipeline|LazyRenderPipeline|lazy-render-pipeline" README.md sub/web-image-util/README.md docs/architecture.md
rg --files sub/web-image-util/src/core
```

현재 메인 처리 구조는 `core/lazy-render-pipeline.ts`의 `LazyRenderPipeline`, `core/single-renderer.ts`, `core/onehot-renderer.ts` 중심이다. 과거의 `pipeline.ts`/`RenderPipeline` 설명을 되살리지 않는다.

## 생성 산출물

공개 API, 패키지 README, 유틸리티 export가 바뀌면 `llm.txt`도 최신 상태여야 한다. 운영 기준은 [llm.txt 운영 가이드](llm-txt.md)를 따른다. `pnpm build`는 `sub/web-image-util/scripts/generate-llm-txt.mjs`를 실행한다.

```bash
pnpm build
git diff -- sub/web-image-util/llm.txt
```

`llm.txt`가 변경되었다면 새 API 설명이 의도대로 반영됐는지 확인한다.

## 배포 전 검증

최소 검증:

```bash
pnpm verify:release
pnpm test:package-subpath
```

변경 범위에 따라 추가 검증:

```bash
pnpm --filter @cp949/web-image-util package
pnpm --filter @cp949/web-image-util test:coverage
pnpm --filter @cp949/web-image-util test:security
```

`test:coverage`가 실패하는 상태에서 배포해야 한다면, 실패 이유와 배포 판단을 릴리스 메모에 명시한다.

## 최종 검색

배포 직전에는 README의 수동 버전 표기, 없어진 구조, 잘못된 명령이 남아 있지 않은지 검색한다.

```bash
rg -n "현재 배포 준비 버전" README.md sub/web-image-util/README.md CLAUDE.md
rg -n "pipeline\\.ts|(^|[^A-Za-z])RenderPipeline([^A-Za-z]|$)|pnpm ci|coverage 목표" README.md sub/web-image-util/README.md docs --glob '!docs/release-checklist.md'
rg -n "npm version" README.md
```

검색 결과가 의도된 문맥인지 직접 읽고 확인한다.
