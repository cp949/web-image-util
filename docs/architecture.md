...
| `src/base/resize-strategy.internal.ts` | 고해상도 전략 seam — `RESIZE_STRATEGY_ADAPTERS` 레지스트리(direct/chunked/stepped/tiled)와 전략별 튜닝 지식(품질 매핑·단계 수·동시성·타일 크기·예상 시간 배수). 전략 추가 = adapter 1개 + 맵 1행 |
