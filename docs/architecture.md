...
| `src/utils/svg-raw-tag-scan.internal.ts` | 파싱 전 SVG 원문에서 시작 태그를 순회하고 속성 이름 집합의 값을 따옴표 방식과 무관하게 추출하는 leaf — "참조 속성" leaf의 raw-text 대응물. 경량 집행 엔진과 intake guard가 공유. on* 판정은 포함하지 않는다(소비자별로 의도적으로 다름) |
...
| `src/base/high-res-detector.internal.ts` | 이미지 크기 분석(`analyzeImage`)과 고해상도 처리 진입 게이트(`shouldUseHighResolutionPath`) 단일 소유 — `HighResolutionProcessor`가 이 게이트를 쓴다(유일한 진입점). `HighResolutionProcessor` 커스텀 픽셀 임계값은 유지한다. 전략 선택 정책(4개 티어 전부·임계값 전부·canvas 크기 가드)은 `src/base/strategy-policy.internal.ts`가 단일 소유 — 이 파일은 `ProcessingStrategy` 재노출과 `selectBalancedStrategy()` 호출만 담당한다. Canvas 최대 안전 치수 사실(UA sniffing·fallback)은 `browser-capabilities/canvas-limits.internal.ts`가 단일 소유 — `getMaxSafeDimension()`은 그 값을 재노출만 한다 |
| `src/core/high-res-processor.ts` | advanced 고해상도 경로의 단일 진입점 — 옛 `HighResolutionDetector`(분석)+`HighResolutionManager`(전략 선택·실행)+`AutoHighResProcessor`(라우팅·표준 경로) 3개 클래스를 통합했다. 이미지 분석·전략 선택(티어별 판정은 `strategy-policy.internal.ts`의 `selectFastStrategy`/`selectHighQualityStrategy`/`selectMemoryEfficientStrategy`에 위임)·메모리 점검(압박 시 `CanvasPool.clear()` + GC 요청)·표준 경로 폴백까지 담당하고 실제 픽셀 처리는 전략 adapter(`resize-strategy.internal.ts`)에 위임 |
| `src/core/performance-utils.ts` | `ResizePerformance`/`fastResize`/`qualityResize`/`autoResize` — `HighResolutionProcessor`를 감싼 advanced 전용 opt-in 표면. 메인 `processImage().resize()` 체인은 `HighResolutionProcessor`를 전혀 호출하지 않으므로, 대용량 이미지 임계값 감지·타일링·메모리 경고가 필요하면 이 표면을 명시적으로 써야 한다. `ResizeConfig`엔 대응 어휘(`priority`)가 없다 — 이름을 맞출 대상이 아니라 의도적으로 분리된 경로다 |
