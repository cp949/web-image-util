...
`ProcessingStrategy` 유니온에서 `CHUNKED`를 제거한다 — `'direct' | 'stepped' | 'tiled'` 3값. `chunked`가 하던 일(작은 타일, 낮은 동시성, 메모리 모니터링 끔)은 `tiledAdapter.execute()` 내부의 preset 선택 로직으로 흡수한다:

```ts
// resize-strategy.internal.ts, tiledAdapter.execute 내부
const preset =
  analysis.estimatedMemoryMB <= 64 // high-res-detector의 기존 MEDIUM 경계 재사용 — 새 숫자를 만들지 않는다
    ? { tileSize: Math.min(2048, analysis.recommendedChunkSize), maxConcurrency: 2, enableMemoryMonitoring: false }
    : { maxConcurrency: quality === 'fast' ? 4 : 2, enableMemoryMonitoring: true };
```

"tiled를 쓰기로 한 뒤 어떤 preset을 쓸지"는 이제 이 한 곳만 판정한다. "애초에 direct/stepped/tiled 중 뭘 쓸지"를 고르는 4개 지점(위 표 + `auto-high-res.ts`)의 상호 불일치는 이 설계로 없어지지 않는다 — 그건 "메모리 예산 모듈" 설계(`docs/design/2026-08-14-memory-budget-module-design.md`)가 이미 명시적으로 비범위 처리한 별도 문제이고, 이번 카드도 그 범위를 넘지 않는다(아래 "비범위" 참고).
...
- `calculateComplexity()`(124행)·`estimateProcessingTime()`(258행대)·`getStrategyDescription()`(322행대)의 `CHUNKED` case를 제거한다. `estimateProcessingTime`의 시간 배수·`getStrategyDescription`의 장단점 텍스트는 TILED case로 흡수하되 문구 자체(설명 텍스트)는 이번 설계에서 확정하지 않는다 — 구현 시 TILED 설명에 "메모리 효율적" 뉘앙스를 병합할지, 기존 TILED 텍스트를 그대로 둘지는 순수 문서 텍스트라 리스크가 낮으므로 담당 태스크에서 판단한다.
...
- `ResizeStrategyAdapter`/`ResizeStrategyInput` 인터페이스는 변경 없음(이미 `analysis: ImageAnalysis` 전체를 받고 있어 preset 판정에 필요한 `estimatedMemoryMB`·`recommendedChunkSize`가 이미 들어온다).
...
- `tiledAdapter.execute()` — `analysis.estimatedMemoryMB`가 64 이하일 때 light preset(`tileSize`가 `recommendedChunkSize` 기준, `maxConcurrency: 2`, `enableMemoryMonitoring: false`)으로 `TiledProcessor.resizeInTiles`를 호출하는지, 64 초과일 때 heavy preset(`enableMemoryMonitoring: true`)으로 호출하는지를 `TiledProcessor.resizeInTiles`를 스파이해 검증.

기존 4개 테스트 파일의 `CHUNKED`/`'chunked'` 기대값을 `TILED`/`'tiled'`로 갱신:
- `tests/unit/base/high-res-detector.test.ts`
- `tests/unit/core/high-res-manager-smart-resize-strategy-jsdom.test.ts`
...
- `getStrategyDescription()`/`estimateProcessingTime()`의 TILED 설명 텍스트를 어떻게 병합할지의 최종 문구 확정(구현 태스크에서 판단, 리스크 낮음).
