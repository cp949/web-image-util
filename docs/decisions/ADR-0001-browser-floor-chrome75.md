...
- 빌드 설정이 빌드 타깃을 `chrome75`로 고정한다 — Chrome 75 이상에서 문법적으로 동작하는 산출물을 만드는 것이 목표다.
...
1. 빌드 타깃(`chrome75`)을 유지한다. 런타임 API 정적 감사와 dist 문법(ES 버전) 감사가 매 CI에서 회귀를 감시한다.
...
3. Chrome 83(Podman) floor validation은 기본 CI 파이프라인(빌드 검증·브라우저 테스트·GitHub Actions) 어디에도 연결하지 않는다. opt-in — 필요할 때만 로컬에서 수동 실행한다.
4. Chrome 75~82 구간은 자동 실행 검증 수단이 없다는 사실을 사용자 문서에 명시한다. 정적 검사(문법·API)만 이 구간을 커버한다.
...
- 신규 코드가 Chrome 75 미만 런타임 API를 가드 없이 추가하면 런타임 API 정적 감사가 CI에서 실패시킨다. 의도적으로 추가하려면 브라우저 호환성 allowlist에 등록해야 한다.
- 이 결정은 배포 대상 패키지에 한정된다. 데모 앱은 대상이 아니다.
