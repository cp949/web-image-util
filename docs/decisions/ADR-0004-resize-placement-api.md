...
- 로드맵 Track 1B. `cover`/`contain`의 배치가 지금은 무조건 중앙 정렬로 하드코딩돼 있다(배치 계산 로직이 `fit`이나 위치 옵션과 무관하게 항상 중앙 좌표를 계산). 특정 피사체를 강조하려면 별도 crop 계산이 필요했다.
- 설계 인터뷰(2026-09-12)로 어휘·좌표계·API 위치를 확정했다. 이 ADR은 결정의 요약이다(영향 절 참조).
...
2. gravity 값 집합은 워터마크/오버레이 배치용 `SimplePosition`(9칸, `custom` 없음, `center` 표기)과 동일하지만, resize 옵션 타입에 독립 타입(`ResizeGravity`)으로 재정의한다. 워터마크/오버레이 배치 모듈을 import하지 않는다 — core 타입 모듈이 부수 기능 모듈에 의존하는 역방향 결합을 피한다.
...
- `SimplePosition` 값 재사용 + 독립 타입 선언: 라이브러리 전체에서 "박스 안 배치"라는 개념의 어휘를 통일하면서도, core 타입 모듈이 워터마크/오버레이 배치 모듈에 의존하는 역방향 결합은 만들지 않는다.
