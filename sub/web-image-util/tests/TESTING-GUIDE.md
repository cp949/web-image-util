...
새 테스트가 *잠그는 것*이 무엇인지로 위치를 정한다.

1. **공개 API에서 관찰 가능한 입출력/부수효과인가?**
   → 예 → **행동 테스트**로 작성한다.
   - 위치: `tests/unit/{processor,presets}/**` 또는 `tests/shortcut/**` 또는 `tests/integration/**`
   - 의도: 그 모듈을 리팩토링해도 외부 행동이 같다면 테스트는 그대로 통과해야 한다.

2. **아니라면, 내부 구현 분기인데 행동 테스트로는 닿을 수 없는가?**
   → 예 → **단위 테스트**로 작성한다.
   - 위치: `tests/unit/{base,core,errors,utils}/**`
   - 검토: "행동으로 닿을 수 없는" 분기가 맞는지 한 번 더 확인. 닿을 수 있으면 1번으로 돌아간다.

3. **그것도 아닌가?**
   → **추가하지 않는다.**
   - 커버리지 숫자만을 위한 테스트는 의도가 없는 테스트다. 리팩토링 시 같이 깨질 가능성만 만든다.

새 공개 API가 추가되면 같은 PR에서 [`docs/testing/coverage-roadmap.md`](../../../docs/testing/coverage-roadmap.md)의 행동 인벤토리 행도 함께 추가한다.
