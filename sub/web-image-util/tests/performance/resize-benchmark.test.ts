...
describe('Resize Performance Benchmarks', () => {
  describe('single image processing', () => {
    slowPerfBenchmark(
      'should resize large images within acceptable time limits',
      async () => {
  ...
  describe('batch processing', () => {
    it('should handle multiple resize operations efficiently', async () => {
      ...
      const promises = Array(10)
        .fill(null)
        .map(() => processImage(testBlob).resize({ fit: 'cover', width: 200, height: 200 }).toBlob());
  ...
  describe('chained operations performance', () => {
    it('should handle chained resize and blur efficiently', async () => {
      ...
      await processImage(testBlob).resize({ fit: 'cover', width: 400, height: 300 }).blur(3).toBlob();
    ...
    it('should handle multiple sequential resizes', async () => {
      ...
      const firstResult = await processImage(testBlob).resize({ fit: 'maxFit', width: 800 }).toBlob();

      // Second resize: first result to 400x400
      await processImage(firstResult.blob).resize({ fit: 'cover', width: 400, height: 400 }).toBlob();
  ...
  describe('memory efficiency', () => {
    ...
    it('should handle large batch without memory overflow', async () => {
      ...
      const promises = Array(50)
        .fill(null)
        .map(() => processImage(testBlob).resize({ fit: 'contain', width: 150, height: 150 }).toBlob());
