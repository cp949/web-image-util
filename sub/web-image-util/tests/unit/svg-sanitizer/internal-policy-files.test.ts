...
import { isSafeInternalReference } from '../../../src/svg-sanitizer/reference-policy.internal';
import { pushUniqueWarning } from '../../../src/svg-sanitizer/warnings.internal';

describe('strict sanitizer 내부 정책 파일', () => {
  it('internal 파일 경로에서 CSS, URI, 경고 helper를 import할 수 있다', () => {
    ...
    expect(isSafeInternalReference('#safe')).toBe(true);
    expect(isSafeInternalReference('https://evil.example/p.svg')).toBe(false);
