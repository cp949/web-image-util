...
import { installImageDataMock, makeResizeResult, mockResize } from './advanced-processor-process.helpers';

describe('AdvancedImageProcessor.processImage resize 옵션', () => {
  let resizeSpy: ReturnType<typeof mockResize>;

  beforeEach(() => {
    resizeSpy = mockResize();
  ...
  describe('resize 옵션만 지정', () => {
    it('결과 canvas 는 resize 스텁 canvas 와 동일 참조다', async () => {
      ...
      resizeSpy.mockResolvedValue(stubResult);
    ...
    it('HighResolutionProcessor.resize 가 source·width·height 를 올바른 순서로 받는다', async () => {
      ...
      expect(resizeSpy).toHaveBeenCalledWith(img, 300, 200, expect.any(Object));
    });
    ...
    it('resize.priority 가 resize 4번째 인자로 전달된다', async () => {
      ...
      expect(resizeSpy).toHaveBeenCalledWith(img, 300, 200, expect.objectContaining({ priority: 'quality' }));
    });
  });
