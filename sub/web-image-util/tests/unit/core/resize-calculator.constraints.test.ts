/**
 * calculateFinalLayout의 극단 입력 처리를 검증하는 단위 테스트다.
 *
...
describe('calculateFinalLayout - 극단 입력', () => {
  describe('초대형 이미지', () => {
    it('8K 해상도(7680x4320)를 처리한다', () => {
      const result = calculateFinalLayout(7680, 4320, {
        fit: 'cover',
        width: 1920,
        height: 1080,
      });
    ...
    it('초대형 이미지(100000x100000)를 처리한다', () => {
      const result = calculateFinalLayout(100000, 100000, {
        fit: 'maxFit',
        width: 1000,
        height: 1000,
      });
  ...
  describe('초소형 이미지', () => {
    it('1x1 픽셀 이미지를 처리한다', () => {
      const result = calculateFinalLayout(1, 1, {
        fit: 'maxFit',
        width: 100,
        height: 100,
      });
    ...
    it('초소형 이미지(10x10)를 처리한다', () => {
      const result = calculateFinalLayout(10, 10, {
        fit: 'contain',
        width: 500,
        height: 500,
      });
  ...
  describe('극단 종횡비', () => {
    it('극단 가로 종횡비(10000:1)를 처리한다', () => {
      const result = calculateFinalLayout(10000, 1, {
        fit: 'contain',
        width: 1000,
        height: 1000,
      });
    ...
    it('극단 세로 종횡비(1:10000)를 처리한다', () => {
      const result = calculateFinalLayout(1, 10000, {
        fit: 'contain',
        width: 1000,
        height: 1000,
      });
    ...
    it('파노라마 이미지(21:9)를 처리한다', () => {
      const result = calculateFinalLayout(2560, 1080, {
        fit: 'cover',
        width: 1920,
        height: 1080,
      });
  ...
  describe('경계 치수', () => {
    ...
    it('소수점 치수를 정수로 변환한다', () => {
      ...
      const result = calculateFinalLayout(1000, 333, {
        fit: 'contain',
        width: 300,
        height: 100,
      });
