  ...
  private static async validateAPICompatibility(report: ValidationReport): Promise<void> {
    try {
      ...
      if (canvas1 instanceof HTMLCanvasElement && canvas1.width === 200 && canvas1.height === 150) {
        ...
        report.failed.push(
          `클래스 API 크기 오류: ${canvas1 instanceof HTMLCanvasElement ? `${canvas1.width}x${canvas1.height}` : '타입 오류'}`
        );
      }
      ...
      if (canvas2 instanceof HTMLCanvasElement && canvas2.width === 200 && canvas2.height === 150) {
        ...
        report.failed.push(
          `함수형 API 크기 오류: ${canvas2 instanceof HTMLCanvasElement ? `${canvas2.width}x${canvas2.height}` : '타입 오류'}`
        );
      }
      ...
      if (canvas3 instanceof HTMLCanvasElement && canvas3.width === 200 && canvas3.height === 150) {
        ...
        report.failed.push(
          `빌더 API 크기 오류: ${canvas3 instanceof HTMLCanvasElement ? `${canvas3.width}x${canvas3.height}` : '타입 오류'}`
        );
      }
