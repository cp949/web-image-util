...
- Changed (**Breaking**): `SvgOptimizationOptions.mergeElements` 필드를 제거했습니다. 타입과
  `SvgOptimizer.getDefaultOptions()`에만 존재했고 `SvgOptimizer.optimize()`의 어떤 단계도 읽지 않는
  유령 필드였습니다. 이 필드를 명시한 객체 리터럴은 초과 속성 검사에 걸립니다.
  해당 줄을 지우세요. `getDefaultOptions().mergeElements`나 옵션 객체의
  `.mergeElements`를 읽던 코드도 컴파일되지 않습니다 —
  해당 참조를 지우세요.
...
- Fixed: `SvgOptimizer`의 기본 옵션(`removeMetadata: true`)이 참조되는 `id`까지
  제거해 렌더링이 깨지던 문제를 수정했습니다. 이제 참조되지 않는 `id`만 제거합니다.
  `href`/`xlink:href`/`src`의
  fragment 참조와 `fill`/`stroke`/`filter`/`clip-path`/`mask`/`marker-*`의 `url(#id)`를 판정합니다.
  - `removeUnusedDefs`가 `xlink:href="#id"` 참조를 놓쳐 사용 중인 `<defs>` 정의를
    제거하던 2차 결함도 해소됩니다.
  - `optimizeGradients`가 중복 정의 병합 시 `fill`/`stroke`만 재작성해 다른 참조에
    삭제된 id를 남기던 결함도 해소됩니다. `href`/`xlink:href`/`src`와 모든 대상
    presentation 속성을 재작성합니다.
  - DOMParser를 쓸 수 없거나 SVG 문자열이 XML로 파싱되지 않으면 `id`를
    하나도 제거하지 않습니다.
    이전에는 이 경우에도 전부 제거했습니다.
