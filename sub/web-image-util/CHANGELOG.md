...
- Added: `box()` 체이닝 메서드를 추가했습니다. `padding`, `background`, `radius`(CSS `border-radius` 의미, 축별 % 지원), `border`(`{ width, color, inset? }`)를 CSS box model로 한 번에 지정합니다. 한 체인에서 한 번만 호출할 수 있으며 체인 위치는 무관합니다(항상 transform → resize → box 순서로 가장 바깥에 적용). 최종 렌더는 여전히 `drawImage()` 1회입니다.
...
- Changed (**Breaking**): `SvgOptimizationOptions.mergeElements` 필드를 제거했습니다. 타입과 `SvgOptimizer.getDefaultOptions()`에만 존재했고 `SvgOptimizer.optimize()`의 어떤 단계도 읽지 않는 유령 필드였습니다. 이 필드를 명시한 객체 리터럴은 초과 속성 검사에 걸립니다 — 해당 줄을 지우세요. `getDefaultOptions().mergeElements`나 옵션 객체의 `.mergeElements`를 읽던 코드도 컴파일되지 않습니다 — 해당 참조를 지우세요.
- Changed (**Breaking**): 브라우저 기능 감지 표면에서 `BrowserCapabilityDetector` 클래스가 제거되었습니다. 모든 메서드가 같은 파일의 모듈 함수로 그대로 위임하던 파사드였습니다(`/filters`가 이미 채택한 "매니저 객체와 모듈 함수 이중 노출 금지" 원칙과 동일하게 정리).
...
- Fixed: `SvgOptimizer`의 기본 옵션(`removeMetadata: true`)이 참조되는 `id`까지 제거해 렌더링이 깨지던 문제를 수정했습니다. 이제 참조되지 않는 `id`만 제거합니다. `href`/`xlink:href`/`src`의 fragment 참조와 `fill`/`stroke`/`filter`/`clip-path`/`mask`/`marker-*`의 `url(#id)`를 판정합니다.
...
- Fixed: advanced 서브패스의 고해상도 처리(`smartResize`, `autoSmartResize`, `fastResize`)에서 `quality:'fast'`/`quality:'high'`와 메모리 압박(저메모리) 경로가 브라우저 Canvas 크기 한계 체크를 건너뛰던 결함을 수정했습니다. 극단적 종횡비(가로/세로 한 축만 매우 크고 전체 메모리 사용량은 작은) 이미지에서 이 세 경로가 타일링 대신 direct/stepped 전략을 골라 Canvas 디코드 한계를 넘을 수 있었습니다. 이제 balanced 전략과 동일하게 캔버스 크기 한계를 공통으로 체크합니다.
  - 같은 이미지가 `AutoHighResProcessor`의 고해상도 게이트(총 픽셀 8MP 초과 또는 스케일 비율 4배 초과)를 안 넘으면 `HighResolutionManager`를 아예 거치지 않고 `standardResize()`로 빠져 위 가드가 안 닿았습니다. 저픽셀이면서 가로/세로 한 축만 극단적으로 큰 이미지(파노라마 등)가 `fastResize()`/`smartResize()`/`autoSmartResize()`에 이 경로로 들어오면 여전히 무조건 direct였습니다. `standardResize()`도 이제 같은 가드를 거쳐 필요하면 tiled로 처리합니다.
