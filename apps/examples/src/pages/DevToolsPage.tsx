...
export function DevToolsPage() {
  ...
  const generateCode = () => {
    const { operation, width, height, format, quality, includeErrorHandling, includeTypeScript, includeComments } = codeGenOptions
  ...
  const getLogIcon = (level: LogEntry['level']) => {
    switch (level) {
      ...
      case 'debug': return <BugIcon color="action" />
                  ...
                  {generatedCode && (
                    <CodeSnippet
                      ...
                      examples={[{
                        ...
                        code: generatedCode,
                        language: codeGenOptions.includeTypeScript ? 'typescript' as const : 'javascript' as const
                      }]}
                    />
                  )}
                </Box>
              ...
              {activeTab === 3 && (
                <Box>
                  ...
                  <Alert severity="info" sx={{ mt: 2 }}>
                    이 정보는 웹 이미지 유틸리티가 사용자 브라우저에서 어떤 기능을
                    사용할 수 있는지 확인하는 데 도움이 됩니다.
                  </Alert>
                </Box>
              )}
            </CardContent>
