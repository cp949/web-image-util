  ...
  const performanceTests: PerformanceTest[] = [
    {
      ...
      category: 'resize',
      operation: () => processImage(sampleImages[testConfig.sampleImage].src)
        .resize(300, 200)
        .toBlob()
    },
    {
                  ...
                  <Stack spacing={2}>
                    {results.some(r => r.avgTime > 200) && (
                      <Alert severity="warning">
                        일부 작업의 처리 시간이 200ms를 초과합니다.
                        Web Worker 사용이나 배치 처리를 고려해보세요.
                      </Alert>
                    )}
                    ...
                    {results.some(r => r.errors > 0) && (
                      <Alert severity="error">
                        일부 작업에서 에러가 발생했습니다.
                        에러 처리 로직과 입력 검증을 강화해보세요.
                      </Alert>
                    )}

                    {memoryStats && memoryStats.delta.used > 50 * 1024 * 1024 && (
                      <Alert severity="info">
                        메모리 사용량이 {formatMemory(memoryStats.delta.used)} 증가했습니다.
                        처리 후 리소스 정리를 확인해보세요.
                      </Alert>
                    )}
                    ...
                    <Alert severity="success">
                      <Typography variant="body2">
                        <strong>성능 개선 팁:</strong><br/>
                        • 대용량 이미지는 단계적으로 처리하세요<br/>
                        • Web Worker를 사용하여 메인 스레드 블로킹을 방지하세요<br/>
                        • 배치 처리 시 Promise.all 대신 순차 처리를 고려하세요<br/>
                        • Canvas 객체를 재사용하여 GC 부담을 줄이세요
                      </Typography>
                    </Alert>
                  </Stack>
                </CardContent>
